...
func _ready(msg: Dictionary) -> Dictionary:
	...
	var bb := "[color=#%s][b]COACHES' RECOMMENDATION[/b][/color]\n" % C_DIM
	...
	if decided == "" and svc != null and svc.is_pending(uid):
		...
		bb += ("[color=#%s]Approve to transform %s immediately (+%d morale; its old " +
			"learnset stays trainable). Postpone and it loses %d morale — the offer " +
			"returns in %d days if the requirements still hold.[/color]\n") % \
		...
		actions.append({"kind": "evo_postpone", "evo_uid": uid, "style": "bad",
			"label": "Postpone (-%d morale)" % EvoSvc.POSTPONE_MORALE_COST})
	else:
		...
		if str(msg.get("decided_on", "")) != "":
			bb += " [color=#%s]on %s[/color]" % [C_DIM, Season.pretty_date(str(msg["decided_on"]))]
		bb += "[/color]\n"
		if word == "postponed" and svc != null:
			bb += "[color=#%s]The coaches will raise it again if %s still qualifies.[/color]\n" % [C_DIM, name]
...
func _stone_hint(msg: Dictionary) -> Dictionary:
	...
	var inst: Dictionary = GameState.squad_member(uid)
	var name := _mon_name(inst, msg)
	var iname := str(DataStore.item(stone).get("name", stone))
	var price := int(DataStore.item(stone).get("price", 0))
	var owned := int(GameState.player_inventory().get(stone, 0))

	var bb := "[color=#%s][b]STAFF REPORT — EVOLUTION ROUTE[/b][/color]\n" % C_DIM
	bb += "[color=#%s]%s[/color]\n\n" % [C_WHITE, str(msg.get("body", ""))]
	if inst.is_empty():
		bb += "[color=#%s]%s is no longer in the squad.[/color]\n" % [C_DIM, name]
		return {"bbcode": bb, "actions": [{"label": "View Squad", "screen": "squad"}], "banner": {}}
	bb += _compare_block(int(msg.get("evo_from", inst.get("species_id", 0))),
		int(msg.get("evo_to", 0)), inst)
	bb += "\n[color=#%s][b]THE STONE[/b][/color]\n" % C_DIM
	if owned > 0:
		bb += "[color=#%s]%d× %s in the storeroom — you can apply it right now. Using a stone needs no further approval.[/color]\n" % \
			[C_GOOD, owned, iname]
	else:
		bb += "[color=#%s]None in stock. The League Store lists the %s at %s.[/color]\n" % \
			[C_WARN, iname, _money(price)]
	var actions: Array = []
	var evolved_already: bool = int(inst.get("species_id", 0)) != int(msg.get("evo_from", inst.get("species_id", 0)))
	if evolved_already:
		bb += "[color=#%s]%s has since evolved — this route is closed.[/color]\n" % [C_DIM, name]
	elif owned > 0 and svc != null:
		actions.append({"kind": "evo_stone", "evo_uid": uid, "item": stone,
			"style": "good", "label": "Use %s now" % iname})
	else:
		actions.append({"label": "Buy in League Store", "screen": "items"})
	actions.append({"label": "View Squad", "screen": "squad"})
	return {"bbcode": bb, "actions": actions, "banner": {}}
...
func _done(msg: Dictionary) -> Dictionary:
	...
	var bb := "[color=#%s][b]EVOLUTION COMPLETE[/b][/color]\n" % C_DIM
	bb += "[color=#%s]%s[/color]\n\n" % [C_WHITE, str(msg.get("body", ""))]
	if from_id > 0 and to_id > 0:
		bb += _compare_block(from_id, to_id, inst)
		bb += ("\n[color=#%s]Its pre-evolution learnset was merged — the old " +
			"techniques stay available on the training ground.[/color]\n") % C_DIM
	var actions: Array = [{"label": "View Squad", "screen": "squad"}]
	if not inst.is_empty():
		actions.append({"label": "Go to Training", "screen": "training"})
	return {"bbcode": bb, "actions": actions, "banner": {}}
...
func _method_block(msg: Dictionary, inst: Dictionary) -> String:
	if inst.is_empty():
		return "[color=#%s]%s is no longer in the squad — the offer lapsed.[/color]\n" % \
			[C_DIM, _mon_name(inst, msg)]
	var svc := _svc()
	var uid := str(inst.get("uid", ""))
	var bb := "[color=#%s][b]WHERE IT STANDS[/b][/color]\n" % C_DIM
	var lv := int(inst.get("level", 1))
	if svc != null:
		bb += "[color=#%s]Lv %d — effective Lv [b]%d[/b] with +%d from %d training development points · morale %d · condition %d%%[/color]\n" % \
	...
	match str(msg.get("evo_method", "")):
		"level":
			bb += "[color=#%s]Route: level threshold reached through matches and training.[/color]\n" % C_DIM
		"development":
			bb += "[color=#%s]Route: development milestone — the staff's long-term work paying off.[/color]\n" % C_DIM
	return bb + "\n"
...
func _compare_block(from_id: int, to_id: int, _inst: Dictionary) -> String:
	...
	var bb := "[color=#%s][b]WHAT CHANGES[/b][/color]\n" % C_DIM
	bb += "[color=#%s][b]%s[/b][/color] [color=#%s](%s · base %d)[/color]  ->  [color=#%s][b]%s[/b][/color] [color=#%s](%s · base %d, %+d)[/color]\n" % \
	...
	if not extra.is_empty():
		bb += "[color=#%s]Unlocks in training: %s[/color]\n" % [C_DIM,
			", ".join(PackedStringArray(extra.slice(0, 8))) + (" +%d more" % (extra.size() - 8) if extra.size() > 8 else "")]
	return bb


func _types(sp: Dictionary) -> String:
	return " / ".join((sp.get("types", []) as Array).map(func(x): return str(x).capitalize()))
...
func _mon_name(inst: Dictionary, msg: Dictionary) -> String:
	...
	var cut := title.find(" is ready")
	if cut < 0:
		cut = title.find(" could evolve")
	return title.substr(0, cut) if cut > 0 else "The Pokémon"
