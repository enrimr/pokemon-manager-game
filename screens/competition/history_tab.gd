...
func refresh() -> void:
	...
	_status.text = tr("Season %d in progress · %d completed season%s on record") % [
		GameState.season_no(), hist.size(), "" if hist.size() == 1 else "s"]
	if hist.is_empty():
		_body.add_child(UI.dim(tr("No season has finished yet. History is written at the end-of-season ceremony:\nleague champions, the %s bracket, the %s winners, Pokémon of the Season and Best Developer\nall land here permanently — and carry across seasons and saves.")
			% [tr(Season.PLAYOFF_NAME), tr(GameState.cup_name())], 13))
...
func _roll_of_honour(hist: Array) -> Control:
	var card := UI.card(tr("%s ROLL OF HONOUR") % tr(Season.INDIGO_TITLE).to_upper())
	card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var body := UI.card_body(card)
	if hist.is_empty():
		body.add_child(UI.dim(tr("Nobody has been crowned yet."), 12))
		return card
	for i in range(hist.size() - 1, -1, -1):
		...
		var season_l := UI.dim(tr("Season %d") % int(e.get("season", i + 1)), 12)
		...
		h.add_child(UI.link(str(ind.get("name", "?")), 13, GOLD.lightened(0.25),
			{"kind": "club", "id": cid}, tr("View club profile")))
		var chip := UI.league_chip(GameState.league_of(cid), 8)
		chip.size_flags_vertical = Control.SIZE_SHRINK_CENTER
		h.add_child(chip)
		h.add_child(UI.dim(tr("def. %s in the Final") % str(ind.get("runner_up_name", "?")), 11))
		body.add_child(h)
	return card


func _season_card(e: Dictionary) -> Control:
	var card := UI.card(tr("SEASON %d · %s to %s") % [int(e.get("season", 1)),
		I18n.pretty_date(str(e.get("start", "?"))), I18n.pretty_date(str(e.get("end", "?")))])
	card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var body := UI.card_body(card)
	...
	var ind: Dictionary = e.get("indigo", {})
	grid.add_child(UI.dim(tr(Season.INDIGO_TITLE), 12))
	...
	for lg in GameState.leagues():
		...
		if lc.is_empty():
			continue
		grid.add_child(UI.dim(tr("%s champions") % tr(str(lg["name"])), 12))
		grid.add_child(_club_line(str(lc.get("club_id", "")),
			tr("%s (%d pts)") % [str(lc.get("name", "?")), int(lc.get("points", 0))], Color.WHITE))
	var cup: Dictionary = e.get("cup", {})
	if str(cup.get("winner", "")) != "":
		grid.add_child(UI.dim(tr("%s winners") % tr(GameState.cup_name()), 12))
	...
	if not p.is_empty():
		grid.add_child(UI.dim(tr("Your season (%s)") % str(p.get("name", "?")), 12))
		grid.add_child(UI.label(tr("%s in the %s · %d pts · %s: %s") % [
			_ord(int(p.get("pos", 0))), tr(GameState.league_name(str(p.get("league", "")))),
			int(p.get("points", 0)), tr(GameState.cup_name()), tr(str(p.get("cup", "?")))], 12,
	...
	if not po.is_empty():
		...
		body.add_child(UI.dim(tr("%s run-in:  %s") % [tr(Season.PLAYOFF_NAME), "   ·   ".join(line)], 11))
	return card
...
func _club_line(cid: String, text: String, col: Color) -> Control:
	...
	h.add_child(UI.monogram(GameState.club(cid), 16, 8))
	h.add_child(UI.link(text, 12, col, {"kind": "club", "id": cid}, tr("View club profile")))
	var chip := UI.league_chip(GameState.league_of(cid), 8)
	chip.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	h.add_child(chip)
	return h
...
func _award_line(a: Dictionary) -> Control:
	...
	h.add_child(UI.link(str(GameState.club(cid).get("short", "?")), 12, TB.COL_TEXT_DIM,
		{"kind": "club", "id": cid}, tr("View club profile")))
	h.add_child(UI.dim(tr("%s avg rating · %d battles") % [I18n.decimal(float(a.get("rating", 0.0)), 2), int(a.get("battles", 0))], 11))
	return h
