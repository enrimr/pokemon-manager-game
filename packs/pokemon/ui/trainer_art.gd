...
## Ash/Gary/Red/Lance, the professors — bundled at res://packs/pokemon/assets/trainers/
...
static func has_art(display_name: String) -> bool:
	return ResourceLoader.exists("res://packs/pokemon/assets/trainers/%s.png" % slug(display_name))
...
static func tex(display_name: String) -> Texture2D:
	...
	if _cache.has(sl):
		return _cache[sl]
	if not ResourceLoader.exists("res://packs/pokemon/assets/trainers/%s.png" % sl):
		return null
	var t: Texture2D = load("res://packs/pokemon/assets/trainers/%s.png" % sl)
	_cache[sl] = t
	return t
