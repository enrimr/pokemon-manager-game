...
## res://packs/pokemon/assets/pokemon/<national-dex-id>.png (gen 1+2, ids 1..251, sourced
...
static func has_art(species_id: int) -> bool:
	return species_id >= 1 and species_id <= 251 \
		and ResourceLoader.exists("res://packs/pokemon/assets/pokemon/%d.png" % species_id)
...
## at res://packs/pokemon/assets/pokemon/front|back/<id>.png — the most modern pixel set
...
static func tex_view(species_id: int, view: String) -> Texture2D:
	...
	if view != "":
		var p := "res://packs/pokemon/assets/pokemon/%s/%d.png" % [view, species_id]
		if ResourceLoader.exists(p):
			t = load(p)
	if t == null and has_art(species_id):
		t = load("res://packs/pokemon/assets/pokemon/%d.png" % species_id)
	_cache[key] = t
	return t
